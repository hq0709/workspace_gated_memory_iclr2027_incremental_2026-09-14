...
\input{tables/tab_naming_recall}
\input{tables/tab_naming_balanced}
...
and assistant generation prefix; Qwen3 thinking is disabled.

\begin{sdabox}{Admission prompt, shared by SDA and the report baseline}
\emph{Passage}
...
\input{tables/tab_adapter_capability}
...
\input{tables/tab_statedness_paired}
...
\input{tables/tab_frozen_batteries}

\input{tables/tab_stress}
\input{tables/tab_bigpool_w24}

\input{tables/tab_sda_budget}

\input{tables/tab_baselines_w24}
...
\input{tables/tab_episode_metrics}
...
\input{tables/tab_within_unstated}
...
+\frac{1450}{2694}\,\mathrm{AUC}_{\mathrm{added}}
=0.627.
...
\input{tables/tab_wording_distill}

\input{tables/tab_wording}
...
\input{tables/tab_stated_control}
...
\input{tables/tab_readout_shuffle}
...
\input{tables/tab_teacher_models}

\input{tables/tab_teacher_models_paired}

Table~\ref{tab:crosspairmodels} gives the paired-passage results for each model.

\input{tables/tab_crosspair_models}
...
363 episodes and 1,607 candidates. Each readout takes its own maximum
over surface forms. The first-token readout is recomputed in the same
measurement call as the two full-candidate aggregations.}
\label{tab:candidatereadout}
\input{tables/tab_candidate_readout}
\end{table}
