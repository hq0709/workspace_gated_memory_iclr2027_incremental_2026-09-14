...
\end{tikzpicture}
\caption{\textbf{SDA training and deployment.} (a) Both the context
$x$ and the candidate set $C$ enter the frozen teacher. Workspace and
likelihood ranks produce top-2 labels for training the admission adapter.
(b) The host supplies context and candidates to the trained importance
prompt, stores the top-$k$ concepts, and later queries its frozen reader.
The future question enters only at recall.}
\label{fig:sda}
\end{figure}
