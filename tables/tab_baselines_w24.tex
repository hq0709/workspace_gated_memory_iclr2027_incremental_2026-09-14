\begin{table}[tbp]
\centering
\caption{Admission policies on Qwen3-8B, blind semantic grading, recall in percent. The standard pool leaves the useful concept as the only candidate the passage never states in 95\% of episodes; the pool with unstated distractors adds four more candidates per episode that the passage also never states. Output likelihood is used directly. Rows above the rule are untrained baselines; dashes denote unmeasured settings.}
\label{tab:baselines}
\small
